...
<div data-css-class="woof_sku_search_container" class="woof_sku_search_container woof_container">
    ...
    <div class="woof_container_inner">
        ...
        if (isset($this->settings['search_by_sku_placeholder_txt']) AND ! isset($placeholder))
        {
            if (!empty($this->settings['search_by_sku_placeholder_txt']))
            {
                $p = $this->settings['search_by_sku_placeholder_txt'];
                $p = WOOF_HELPER::wpml_translate(null, $p);
                $p = __($p, 'woocommerce-products-filter');
            }


            if ($this->settings['search_by_sku_placeholder_txt'] == 'none')
            {
                $p = '';
            }
        }
