...
* Category Video Taxanomy

* 

*/

    class Video extends Ahlu_category{
        ...
        public function index(){
		   ...
		   $this->Video_model = Ahlu::Call("Video_model")->load(); 

		   $this->template->assign("meta",$this->Video_model->SEO());
		   $this->template->assign("content",$this->loader->view("ahlu-{$this->post_type}",array("WP_enable"=>$this->enWP,"title"=>ucfirst($this->class)."s","category"=>$this->Video_model),true));
        ...
        public function post($id){ 
           ...
           $this->post_model = Ahlu::Call("Video_item_model")->load($this->post); 
		...
		public function archive($year=0,$moth=0,$day=0){
			...
			$this->Video_model = Ahlu::Call("Video_model"); 
			$this->template->assign("header",$this->loader->view($this->pathTheme."/compoment/header.php",array("SEO"=>null),true));
			
			if($year>0 && $moth==0 && $day==0){	
				$data = $this->Video_model->archiveByYear($year);
				$title = ucfirst($this->class)." {$this->onView} {$year}:";
			}else if($year>0 && $month>0 && $day==0){
				$data = $this->Video_model->archiveByYearMonth($year,$month);

				$title = ucfirst($this->class)." {$this->onView} on {$month}/{$year}:";
			}else if($year>0 && $month>0 && $day>0){
				$data = $this->Video_model->archiveByYearMonthDate($year,$month,$day);
				$title = ucfirst($this->class)." {$this->onView} on {$month}/{$day}/{$year}:";
			}	
			//
			$this->template->assign("content",$this->loader->view("ahlu-{$this->post_type}-{$this->onView}",array("WP_enable"=>$this->enWP,"title"=>$title,"data"=>$data,"category"=>$this->Video_model),true));
