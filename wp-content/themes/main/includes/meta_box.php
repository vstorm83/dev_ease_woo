...
class RW_Meta_Box {
	...
	function check_field_date() {
		if ($this->has_field('date') && $this->is_edit_page()) {
			...
			wp_enqueue_style('rw-jquery-ui-css', 'http://code.jquery.com/ui/1.10.3/themes/smoothness/jquery-ui.css');
			wp_enqueue_script('rw-jquery-ui', 'http://code.jquery.com/ui/1.10.3/jquery-ui.js', array('jquery'));
	...
	function show_field_image($field, $meta) {
		...
		if (!empty($meta)) {
			...
			$attachs = get_posts(array(
				...
				'post_type' => 'attachment',
				'post_parent' => $post->ID,
				'post_mime_type' => 'image,application', // get attached images only
				'output' => ARRAY_A
			));
			...
			foreach ($attachs as $att) {
				...
				//echo "<pre>";
					//print_r($att);
			$type = get_post_mime_type( $att->ID );
			//echo $type,"<br/>";
			switch( $type ) {  
    case 'application/zip' : 
     ...
     case 'application/extension':  ?>  
         ...
         <?php 
         $b = " ";
         $a= wp_get_attachment_url($att->ID );
           echo "<div style='margin: 0 10px 10px 0; float: left'>" .$att->post_title." :
".wp_get_attachment_url( $att->ID )."  

           				
							<a class='rw-delete-file' href='javascript:void(0)' rel='{$post->ID}!{$field['id']}!{$att->ID}!{$a}!{$a}!{$nonce}'>Delete</a></div>";
							 ?> 
    <?php break;  
